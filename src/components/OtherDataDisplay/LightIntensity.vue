...
export default{
            ...
            fetch("http://175.178.67.109:8801/swagger/getQingdaoAnalySensor", requestOptions)
              ...
              .then(result => {
                ...
                if(no2<=100&& no2 >=0){
                    ...
                    console.log(gcj_lng,gcj_lat);
                    })

        //             var content ='<div id="echartContainer" style="width: 700px;height:500px;"></div><table border="1"><tr><th>污染物</th><td>光照强度</td><td>噪声</td><td>PH</td></tr><tr><th>单位</td><td>1E-3 LUX</td><td>dB</td><td>无</td></tr></table>'
    ...
    watch:{
        light_ShowOrNot(newValue){
            if(newValue){
                this.light_show()
                console.log("<LightIntensity>按钮打开");
            }else{
                this.delLayer()
                console.log("<LightIntensity>按钮关闭");
            }
        }
    },
    created(){
