...
export default{
            ...
            myHeaders.append("User-Agent", "Apifox/1.0.0 (https://apifox.com)");
              ...
              .then(result => {
                ...
                function displayWithDelay(list,index){
                    if(index>=list.length || that.rangeSO2_ShowOrNot==false){
                        return 
                    }
                    setTimeout(function(){
    ...
    watch:{
        rangeSO2_ShowOrNot(newValue){
        if (newValue){
            
            this.range_SO2_show()

          console.log("<Range_SO2>按钮开关：true");
        }else{
          this.delLayer()
          
          console.log("<Range_SO2>按钮开关：false   WebSocket：关闭");
        }
      }
    }
}
