...
export default{
            ...
            myHeaders.append("User-Agent", "Apifox/1.0.0 (https://apifox.com)");
              ...
              .then(result => {
                ...
                function displayWithDelay(list,index){
                    if(index>=list.length || that.rangeCO_ShowOrNot==false){
                        return 
                    }
                    setTimeout(function(){
    ...
    watch:{
        ...
        if (newValue){
          ...
          console.log("<Range_CO>按钮开关：true");
        }else{
          this.delLayer()
          
          console.log("<Range_CO>按钮开关：false   WebSocket：关闭");
        }
      }
    }
}
