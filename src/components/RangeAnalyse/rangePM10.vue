...
export default{
            ...
            myHeaders.append("User-Agent", "Apifox/1.0.0 (https://apifox.com)");
              ...
              .then(result => {
                ...
                function displayWithDelay(list,index){
                    if(index>=list.length || that.rangePM10_ShowOrNot==false){
                        return 
                    }
                    setTimeout(function(){
    ...
    watch:{
        rangePM10_ShowOrNot(newValue){
        if (newValue){
            
            this.range_pm10_show()

          console.log("<Range_PM10>按钮开关：true");
        }else{
          this.delLayer()
          
          console.log("<Range_PM10>按钮开关：false   WebSocket：关闭");
        }
      }
    }
}
