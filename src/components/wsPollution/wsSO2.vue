<template>
  ...
  export default{
    ...
    methods:{
          ...
          myHeaders.append("User-Agent", "Apifox/1.0.0 (https://apifox.com)");
            ...
            //       that.overlayGroup_car =new AMap.OverlayGroup(car_list)
                ...
                for(var i=0;i<obj.length;i++){
                  ...
                  var so2 = obj[i].so2
                  var evaluate_So2 = obj[i].evaluateSo2
                  
  
                  if(so2<=400 && evaluate_So2<=20){
                ...
                marker_list.push(circleMarker)
                  ...
                  }else if(so2<=400 && evaluate_So2>=20){
                    var circleMarker = new AMap.CircleMarker({
                    center: new AMap.LngLat(gcj_lng,gcj_lat),
                    // 半径
                ...
                marker_list.push(circleMarker)
                  }else if(500>=so2 && so2>=400 && evaluate_So2<=20){
                    var circleMarker = new AMap.CircleMarker({
                    center: new AMap.LngLat(gcj_lng,gcj_lat),
                    // 半径
                ...
                marker_list.push(circleMarker)
                  }else if(so2>=500 && evaluate_So2>=0){
                    var circleMarker = new AMap.CircleMarker({
                    center: new AMap.LngLat(gcj_lng,gcj_lat),
                    // 半径
        ...
        wsOpen(e){
            console.log('wsSO2:FE WebSocket open',e);
        },
        wsClose(e){
            console.log('wsSO2:FE WebSocket close',e);
        },
        wsError(e){
            console.log('wsSO2:FE WebSocket Error',e);
        },
        wsMessage(e){
          var that = this
          ...
          var that = this
            ...
            if(("The latest task data has been uploaded")in msg){
              ...
              // ws中：so2插值
              var ws_so2_evaluate=list1[5]
              ws_so2_evaluate = parseFloat(ws_so2_evaluate.slice(5,ws_so2_evaluate.length))
              ...
              // ws中取得的so2真实值
              var ws_so2 = list2[5]
              ws_so2=parseFloat(ws_so2.slice(5,ws_so2.length))
              ...
              var added_marker_list = that.map.getAllOverlays()
                ...
                for(var j=0;j<added_marker_list.length;j++){
                  ...
                  }else if(ws_so2>=500 && ws_so2_evaluate>=0){
            ...
            var content ='<div id="echartContainer" style="width: 600px;height:400px;"></div><table border="1"><tr><th>污染物</th><td>SO2(二氧化硫)</td><td>NO2(二氧化氮)</td><td>O3(臭氧)</td><td>PM2.5</td><td>so2</td><td>CO(一氧化硫)</td></tr><tr><th>单位</td><td>μg/m3</td><td>μg/m3</td><td>μg/m3</td><td>μg/m3</td><td>μg/m3</td><td>mg/m3</td></tr></table>'
    ...
    watch:{
        ...
        if (newValue){
          ...
          // 发送ws数据
          this.sendMsg()
          console.log("<wsSO2>按钮开关：true");
        }else{
          this.delLayer()
          clearInterval(this.clock)
          console.log("<wsSO2>按钮开关：false   WebSocket：关闭");
        }
      }
    }
