...
export default{
  ...
  methods:{
          ...
          fetch("http://175.178.67.109:8801/swagger/pollutionWarning", requestOptions)
            ...
            var car1 = new AMap.Marker({
              ...
              for(var i=0;i<obj.length;i++){
                ...
                // gcj_lng/gcj_lat/pm10
                var gcj_lng = obj[i].longitude
                var gcj_lat = obj[i].latitude
                var pm10 = obj[i].pm10
                var evaluate_pm10 = obj[i].evaluatePm10
                

                if(pm10<=120 && evaluate_pm10<=10){
              ...
              marker_list.push(circleMarker)
                ...
                }else if(pm10<=120 && evaluate_pm10>=10){
                  var circleMarker = new AMap.CircleMarker({
                  center: new AMap.LngLat(gcj_lng,gcj_lat),
                  // 半径
              ...
              marker_list.push(circleMarker)
                }else if(150>=pm10 && pm10>=120 && evaluate_pm10<=10){
                  var circleMarker = new AMap.CircleMarker({
                  center: new AMap.LngLat(gcj_lng,gcj_lat),
                  // 半径
              ...
              marker_list.push(circleMarker)
                }else if(pm10>=150 && evaluate_pm10>=0){
                  var circleMarker = new AMap.CircleMarker({
                  center: new AMap.LngLat(gcj_lng,gcj_lat),
                  // 半径
      ...
      wsOpen(e){
          console.log('wsPM10:FE WebSocket open',e);
      },
      wsClose(e){
          console.log('wsPM10:FE WebSocket close',e);
      },
      wsError(e){
          console.log('wsPM10:FE WebSocket Error',e);
      },
      wsMessage(e){
        var that = this
        ...
        var that = this
          ...
          if(("The latest task data has been uploaded")in msg){
            ...
            // ws中：pm2.5插值
            var ws_pm10_evaluate=list1[3]
            ws_pm10_evaluate = parseFloat(ws_pm10_evaluate.slice(6,ws_pm10_evaluate.length))
            ...
            // ws中取得的pm2.5真实值
            var ws_pm10 = list2[3]
            ws_pm10=parseFloat(ws_pm10.slice(6,ws_pm10.length))
            ...
            var added_marker_list = that.map.getAllOverlays()
              ...
              for(var j=0;j<added_marker_list.length;j++){
                ...
                }else if(ws_pm10>=150 && ws_pm10_evaluate>=0){
  ...
  watch:{
    wsPm10_ShowOrNot(newValue){
      if (newValue){
        ...
        // 发送ws数据
        this.sendMsg()
        console.log("<wsPm10.0>按钮开关：true");
      }else{
        this.delLayer()
        clearInterval(this.clock)
        console.log("<wsPm10.0>按钮开关：false   WebSocket：关闭");
      }
    }
  }
