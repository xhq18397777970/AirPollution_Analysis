  ...
  export default{
    ...
    methods:{
        wsCo_show(){
          ...
          myHeaders.append("User-Agent", "Apifox/1.0.0 (https://apifox.com)");
            ...
            fetch("http://175.178.67.109:8801/swagger/getQingdaoAnalySensor", requestOptions)
              ...
              .then(result => {
                ...
                for(var i=0;i<obj.length;i++){
                  ...
                  var co = obj[i].co
                  var evaluate_co = obj[i].evaluateCo
                  
  
                  if(co<=6 && evaluate_co<=1){
                ...
                marker_list.push(circleMarker)
                  }else if(10>=co && co>=6 && evaluate_co<=1){
                    var circleMarker = new AMap.CircleMarker({
                    center: new AMap.LngLat(gcj_lng,gcj_lat),
                    // 半径
        ...
        wsOpen(e){
            console.log('wsCO:FE WebSocket open',e);
        },
        wsClose(e){
            console.log('wsCO:FE WebSocket close',e);
        },
        wsError(e){
            console.log('wsCO:FE WebSocket Error',e);
        },
        wsMessage(e){
          var that = this
          ...
          var that = this
            ...
            if(("The latest task data has been uploaded")in msg){
              ...
              // ws中：co插值
              var ws_co_evaluate=list1[7]
              ws_co_evaluate = parseFloat(ws_co_evaluate.slice(4,ws_co_evaluate.length))
              ...
              // ws中取得的co真实值
              var ws_co = list2[7]
              ws_co=parseFloat(ws_co.slice(4,ws_co.length))
              ...
              // 获取当前地图上的marker覆盖物
                ...
                for(var j=0;j<added_marker_list.length;j++){
                  ...
                  }else if(ws_co>=10 && ws_co_evaluate>=0){
    ...
    watch:{
        ...
        if (newValue){
          ...
          // 发送ws数据
          this.sendMsg()
          console.log("<wsCO>按钮开关：true");
        }else{
          this.delLayer()
          clearInterval(this.clock)
          console.log("<wsCO>按钮开关：false   WebSocket：关闭");
        }
      }
    }
