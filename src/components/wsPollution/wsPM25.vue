    ...
    methods:{
        wsPM25_show(){
          ...
          myHeaders.append("User-Agent", "Apifox/1.0.0 (https://apifox.com)");
            ...
            fetch("http://175.178.67.109:8801/swagger/pollutionWarning", requestOptions)
              ...
              var car1 = new AMap.Marker({
                ...
                for(var i=0;i<obj.length;i++){
                  ...
                  // gcj_lng/gcj_lat/pm2p5
                  var gcj_lng = obj[i].longitude
                  var gcj_lat = obj[i].latitude
                  var pm2p5 = obj[i].pm2p5
                  var evaluate_pm2p5 = obj[i].evaluatePm2p5
                  

                  if(pm2p5<=60 && evaluate_pm2p5<=5){
                ...
                marker_list.push(circleMarker)
                  ...
                  }else if(pm2p5<=60 && evaluate_pm2p5>=5){
                    var circleMarker = new AMap.CircleMarker({
                    center: new AMap.LngLat(gcj_lng,gcj_lat),
                    // 半径
                ...
                marker_list.push(circleMarker)
                  }else if(75>=pm2p5 && pm2p5>=60 && evaluate_pm2p5<=5){
                    var circleMarker = new AMap.CircleMarker({
                    center: new AMap.LngLat(gcj_lng,gcj_lat),
                    // 半径
                ...
                marker_list.push(circleMarker)
                  }else if(pm2p5>=75 && evaluate_pm2p5>=0){
                    var circleMarker = new AMap.CircleMarker({
                    center: new AMap.LngLat(gcj_lng,gcj_lat),
                    // 半径
            ...
            //           console.log('实际经纬度：'+gcj_lng+','+gcj_lat);
        ...
        //         // time =  month + "月" + date + "日" + " " + hour + ":" + "00" ;

        //         let currentTime=getTimeStr(month,date,hour,hour+1)
        //         let beforeOneDay=getTimeStr(month,date-1,hour,hour2)

        //         function getTimeStr(month,date,hour,hour2){
        //           month = month + 1;
        //           if (month < 10) month = "0" + month;
        //           if (date < 10) date = "0" + date;
        //           if (hour < 10) hour = "0" + hour;
        //           if (hour2 < 10) hour2 = "0" + hour2;
        //           return month + "月" + date + "日" + " " + hour + ":00-"+hour2+":00" ;
        //         }
              ...
              {
              type: 'category',
              // data: ['9月9日8:00点-9:00', '9月9日11:00点-12:00', '9月10日8:00点-9:00', '9月10日11:00点-12:00','9月11日8:00点-9:00',
              //       '9月11日11:00点-12:00', ],
              data: ['9月9日10:00点-12:00',  '9月10日10:00点-12:00','9月11日9:00点-11:00', ]
              }
              ],
              yAxis: [
              {
        ...
        wsOpen(e){
            console.log('wsPm2.5：FE WebSocket open',e);
        },
        wsClose(e){
            console.log('wsPm2.5：FE WebSocket close',e);
        },
        wsError(e){
            console.log('wsPm2.5：FE WebSocket Error',e);
        },
        wsMessage(e){
          var that = this
          ...
          var that = this
            ...
            if(("The latest task data has been uploaded")in msg){
              ...
              // ws中：pm2.5插值
              var ws_pm2p5_evaluate=list1[2]
              ws_pm2p5_evaluate = parseFloat(ws_pm2p5_evaluate.slice(7,ws_pm2p5_evaluate.length))
              ...
              // ws中取得的pm2.5真实值
              var ws_pm2p5 = list2[2]
              ws_pm2p5=parseFloat(ws_pm2p5.slice(7,ws_pm2p5.length))
    ...
    watch:{
      wsPm25_ShowOrNot(newValue){
        if (newValue){
          ...
          console.log("<wsPm2.5>按钮开关：true");
        }else{
          this.delLayer()
          clearInterval(this.clock)
          console.log("<wsPm2.5>按钮开关：false   WebSocket：关闭");
        }
      }
    }
